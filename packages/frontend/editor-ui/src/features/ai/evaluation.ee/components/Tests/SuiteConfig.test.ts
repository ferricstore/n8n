...
describe('SuiteConfig', () => {
	...
	it('removes a metric from the suite when its chip remove is clicked', async () => {
		const store = setup();
		store.selectedMetricKeys = ['correctness'];
		const { getByTestId } = renderComponent();
		await userEvent.click(getByTestId('tests-suite-config-metric-remove-correctness'));
