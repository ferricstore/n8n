...
import { CANNED_METRICS, cannedMetricLabelKey } from '../../evaluation.constants';
import {
	extractCaseAnswer,
	getMetricCategory,
...
const runDateLabel = computed(() => {
	...
	if (!run) return '';
	const d = new Date(run.runAt ?? run.createdAt);
...
const metricBadges = computed<MetricBadge[]>(() => {
	...
	return getUserDefinedMetricNames(metrics).map((key) => {
		...
		const isAiJudged = getMetricCategory(key) === 'aiBased';
		if (isAiJudged) {
			const score = Math.round(value);
			return { key, label, text: `${score}/5`, passed: score >= 3 };
		}
		const pct = Math.round(value * 100);
		return { key, label, text: `${pct}%`, passed: value >= 0.5 };
	});
});
...
watch(
	[expanded, () => caseExecution.value?.executionId],
	([isExpanded, executionId]) => {
		if (!isExpanded || !executionId) return;
