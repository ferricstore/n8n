...
function canConnectPrivateCredential(credentialType: string): boolean {
	...
	return permissions.connect === true || permissions.update === true;
}

async function onConnectFromRow(credentialType: string): Promise<void> {
	// Users with edit rights go through the credential modal; end-users
	// without edit rights connect their own account via OAuth directly.
	if (canEditPrivateCredential(credentialType)) {
