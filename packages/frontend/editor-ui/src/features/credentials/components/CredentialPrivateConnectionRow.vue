...
import { N8nActionDropdown, N8nButton, N8nIcon, N8nTooltip } from '@n8n/design-system';
			...
			<div :class="$style.textGroup">
				...
				<span :class="$style.status">
					<N8nTooltip :disabled="!isConnected || !connectedAccountName" placement="top">
						<!-- Keep the account name unbreakable so the tooltip wraps
							between words, not mid-address -->
						<template #content>
...
/* Wrap between words only — never mid-word/mid-address */
.tooltipAccount {
	display: block;
	word-break: normal;
...
/* Match the Figma menu spec: 4px corner radius (the menu's built-in
   inset shadow ring already draws the 1px border) */
.connectedDropdown {
...
/* Icon-to-label spacing per Figma: 4px total (drop the item gap, let the
   icon wrapper's margin carry the spacing) */
...
/* Destructive action: highlight in red on hover */
